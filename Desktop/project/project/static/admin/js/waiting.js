...
var currow	
	 $('#myTable').on('click', '#update_btn', function() {
		...
		var w_id = currow.find("#w_id").text();
...
var book_id
	$('#myTable').on('click', '#delete_btn', function() {
			...
			w_id = currow2.find("#w_id").text();
			$("#model3 #w_id").text(w_id);
	});
$('#delete').click(function(){
	      var csrftoken = getCookie('csrftoken');
      $.ajax({
        method:'post',
        url:'/admin/waiting_delete/',
        data : {csrfmiddlewaretoken : csrftoken ,w_id : w_id},
...
$('#update').click(function(){
	...
	if(seat_no == ""){
		...
		$('#model1 #alert-6').text('');
          ...
          $.ajax({
            method:'post',
            url:'/admin/waiting_update/',
            data:{csrfmiddlewaretoken: csrftoken ,w_id:w_id,seat_no :seat_no ,c_id : customer_id ,t_id : time_table_id ,journey_date : journey_date , journey_time : journey_time},
            success:function(msg){     
					$("#model2 #cancel").click();
					Materialize.toast('Update Success', 5000);
					currow.find("#w_id").text();
					currow.find("#seat_no").text(seat_no);
					currow.find("#customer_id").text(customer_id);
					currow.find("#time_table_id").text(time_table_id);
					currow.find("#jounrey_date").text(journey_date);
